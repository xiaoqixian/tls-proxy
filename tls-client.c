...
/*
 *  All arguments of callback functions related to TCP
 *  are actually of TCPArg* type. 
 *  It's just for compatibility to define tha arg as void* type.
 */
typedef struct {
    char                addr[16];
    char                port[6];
...
void *service(void *arg) {
    ...
    int optval = 1;
    struct evconnlistener *tcplistener = NULL; //A callback that we invoke when a listener has a new connection.
    ...
    if (arg != NULL) {
        ...
        /*setsockopt is used to pass some miscellaneous to a particular layer to request some service.*/
...
void tcp_newconn_cb(struct evconnlistener *listener, int sock, struct sockaddr *addr, int addrlen, void *arg) {
    ...
    getsockname(sock, (struct sockaddr *)&destaddr, (socklen_t *)&addrlen); //returns the current address to which that the sock fd is bound, in the buffer pointed by destaddr.
...
void tcp_read_cb(struct bufferevent *bev, void *arg) {
    ...
    evbuffer_add_buffer(output, input); //destructively move all data from one evbuffer to another evbuffer.
